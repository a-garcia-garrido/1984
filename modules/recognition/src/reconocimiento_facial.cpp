...
void RecortaCara(const Mat& face, Mat& warped, Rect leftEye, Rect rightEye)
{
	...
	// Obtiene el �ngulo entre los dos ojos
	double dy = (right.y - left.y);
	double dx = (right.x - left.x);
	double len = sqrt(dx*dx + dy*dy);
	double angle = atan2(dy, dx) * 180.0 / CV_PI;

	// Le decimos que, en una imagen escalada, la posici�n ideal para el ojo derecho debe ser (0.19, 0.14)
	const double DESIRED_RIGHT_EYE_X = (1.0f - DESIRED_LEFT_EYE_X);

	//Obtiene la cantidad de escalado que debemos aplicar a la imagen para que sea del tama�o que deseamos
	double desiredLen = (DESIRED_RIGHT_EYE_X - DESIRED_LEFT_EYE_X) * FaceWidth;
	double scale = desiredLen / len;

	// Obtiene la matriz necesaria para rotar y escalar la cara de la imagen hasta el �ngulo y tama�o deseado
